...
class SearchInKnowledgeBaseTool(BaseTool, ABC):
    ...
    @property
    def knowledge_opensearch_collection_url(self):
        return os.environ.get("KNOWLEDGE_OPENSEARCH_COLLECTION_URL")

    @abstractmethod
    def knowledge_opensearch_index_name(self):
        raise NotImplementedError

    def get_vector_store(self):
        return OpenSearchVectorSearch(
            opensearch_url=self.knowledge_opensearch_collection_url,
            index_name=self.knowledge_opensearch_index_name(),
...
class SearchInOpensearchDocumentationInput(BaseModel):
    phrase: str = Field(description="Phrase to search for (similarity search) in OpenSearch documentation")


class SearchInOpensearchDocumentationTool(SearchInKnowledgeBaseTool):
    name: str = "SearchInOpensearchDocumentation"
    description: str = "Searches for an additional information in the OpenSearch documentation"
    args_schema: Optional[ArgsSchema] = SearchInOpensearchDocumentationInput
    return_direct: bool = True

    def knowledge_opensearch_index_name(self):
        return os.environ['KNOWLEDGE_OPENSEARCH_DOCS_INDEX_NAME']
